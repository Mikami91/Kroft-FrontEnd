...
// Actions Creators
import { warningSnackbar } from "../../redux/actions/creators/snackbarCreator";

function RestaurantForm(props) {
  const { superadmin, company, fetching } = props;
  // Local State
  ...
  const current_image =
    Object.keys(company).length !== 0
      ? `${API}images/companies/${company.photo}`
      : image;
  ...
  const handleCreate = (e) => {
    ...
    if (superadmin.email !== undefined) {
      ...
      warningSnackbar("Usuario no autorizado.");
    }
  };
          ...
          >
            ...
            <CardIconActions>
              ...
              <IconButton
                ...
                onClick={() => {
                  document.getElementById("restaurant-logo-file").click();
                }}
              >
                <label>
                  <AddAPhotoIcon />
...
const mapStateToProps = (state) => {
  const { superadmin, company } = state;
  return {
